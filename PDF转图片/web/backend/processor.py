    ...
    try:
        for sheet_index, ws in enumerate(wb.worksheets):
            ...
            for image_index, image in enumerate(getattr(ws, '_images', [])):
                ...
                col = getattr(getattr(anchor, '_from', None), 'col', 0)
                image_path = os.path.join(image_dir, 'sheet_%03d_image_%05d.png' %
...
def process_receipt_workbooks(workbook_paths, out_dir, progress=None):
    ...
    image_dir = os.path.join(out_dir, 'workbook_images')
    os.makedirs(image_dir, exist_ok=True)
    output = os.path.join(out_dir, '赵淑华发票识别结果.xlsx')
    total = max(len(workbook_paths), 1)
    image_records = []
    for index, path in enumerate(workbook_paths, 1):
        if os.path.isfile(path):
            image_records.extend(_workbook_image_items(path, image_dir))
        report(index, total, '正在提取表格图片 %d/%d：%s' %
               (index, total, os.path.basename(path)))
    if not image_records:
        raise RuntimeError('原始表格中没有找到嵌入图片')
...
def process_mode2(pdf_paths, out_dir, inv_type, progress=None):
    ...
    inv_type: '1' canexs | '2' 精准 | '3' 创时亚马逊卡派 | '4' 创时卡派 | '5' 创时清关费 | '6' 创时附加费 | '7' MAX萨凡纳 | '8' MAX纽约 | '9' AA | '10' JCK | '11' MKK | '12' DINO | '13' EYNEX | '14' DRAYEASY。
    返回生成的 Excel 绝对路径。
    """
    def report(cur, tot, msg):
        if progress:
    ...
    for i, pdf in enumerate(pdf_paths, 1):
        if not os.path.isfile(pdf):
            ...
            continue
        if inv_type == '14':
            rows, pg, sk = tool.extract_drayeasy_from_pdfs([pdf])
        elif inv_type == '13':
