...
def create_task(pdf_files, mode, inv_type, layout='v', start_cell='A1',
    ...
    for i, (orig_name, data) in enumerate(pdf_files, 1):
        ...
        if not safe.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.xlsx', '.xlsm')):
            safe += '.pdf'
...
def create_report_task(filename, data, sheet_name, report_profile='bundle'):
    """创建报表组 Excel 处理任务。"""
    task_id = _make_task_id()
    task_dir = os.path.join(_TMP_ROOT, task_id)
    in_dir = os.path.join(task_dir, 'in')
    ...
    while True:
        ...
        try:
            if mode == '4step':
                ...
                if task.get('report_profile') == 'bundle':
                    result, result_name = report_csv_cli.run_web_csv_bundle(task, progress)
                    task['input_path'] = result
                    task['step'] = task.get('max_step', 10)
                    task['current'] = task.get('max_step', 10)
                    report_step = task.get('max_step', 10)
                else:
                ...
                if all(os.path.splitext(path)[1].lower() in ('.xlsx', '.xlsm') for path in pdfs):
                    result = processor.process_receipt_workbooks(pdfs, task['out_dir'], progress)
                else:
