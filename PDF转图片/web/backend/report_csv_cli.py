...
REPORT_PROFILES = {
    ...
    'no_salesperson_cost': {
        ...
        'category': '货物类型', 'unit': '业务员成本单价', 'category_zero': '无业务员成本',
...
def process_step(src, dst, log_path, step, header, idx, profile='no_receivable'):
    if profile == 'no_salesperson_cost':
        rules = {
          1: lambda v: True,
          2: lambda v: not is_signed_in(v[idx['操作状态']]),
          3: lambda v: text(v[idx['业务员成本单价']]) != '' and number(v[idx['业务员成本单价']]) < 1,
          4: lambda v: not any(k in text(v[idx['客户简称']]) for k in EXCLUDED),
          5: lambda v: '华南KA' not in text(v[idx['业务员']]),
          6: lambda v: not remark_remove(v[idx['自定义备注']]),
          7: lambda v: not ('整柜' in text(v[idx['销售产品']]) and number(v[idx['应收金额']]) > 10000),
        }
    else:
        rules = {
          1: lambda v: not is_signed_in(v[idx['操作状态']]),
...
def run_web_csv_bundle(task, progress=None):
    ...
    for profile in ('no_receivable', 'no_salesperson_cost'):
        ...
        current = csv0
        first_step = 1 if profile == 'no_receivable' else 2
        max_filter = 7
        for step in range(first_step, max_filter + 1):
