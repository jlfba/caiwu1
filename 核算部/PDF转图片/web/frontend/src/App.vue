...
function addFiles(list) {
  ...
  for (const f of list) {
    ...
    const allowed = ['6', '7', '8'].includes(effectiveMode.value)
      ...
      <div class="step-body">
        <div class="upload-pane">
          <h2 class="step-title">{{ effectiveMode === '6' && receiptSubtype !== 'invoice' ? '上传' + paymentLabel + '凭证' : '上传 PDF 文件' }}</h2>
           <p class="step-sub">{{ effectiveMode === '6' && receiptSubtype !== 'invoice' ? '支持图片、PDF，也可以直接拖入文件夹' : ['1', '2', '5', '9'].includes(effectiveMode) ? '支持多选或拖入文件夹，自动收集其中的 PDF' : '支持多选，一次拖入全部发票' }}</p>

           <UploadArea :disabled="submitting" :count="files.length" :allow-directories="['1', '2', '5', '6', '9'].includes(effectiveMode)" :accept="effectiveMode === '6' ? '.pdf,.png,.jpg,.jpeg' : '.pdf'" :file-label="effectiveMode === '6' && receiptSubtype !== 'invoice' ? paymentLabel + '凭证' : 'PDF'" @add="addFiles" @remove="removeFile" @clear="clearFiles">
