...
def create_report_task(filename, data, sheet_name):
    ...
    task = {
        ...
        'status': 'pending', 'current': 0, 'total': 9, 'step': 1, 'max_step': 9,
...
def continue_report_task(task_id):
    with _LOCK:
        ...
        if not task or task.get('status') != 'paused':
            return False
        if task.get('step', 1) >= task.get('max_step', 9):
            return False
        task['step'] += 1
        task['status'] = 'pending'
        task['current'] = 0
    ...
    while True:
        ...
        def progress(cur, tot, msg):
            if mode == '4step':
                ...
                task['total'] = task.get('max_step', 9)
            else:
                task['current'] = cur
                task['total'] = tot
            task['message'] = msg
        ...
        try:
            ...
            if mode == '4step':
                task['status'] = 'paused' if task.get('step', 1) < task.get('max_step', 9) else 'done'
            else:
                task['status'] = 'done'
