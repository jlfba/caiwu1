...
> 局域网其他电脑访问：`http://<本机IP>:8000`（后端已监听 0.0.0.0，首次可能弹出防火墙提示点"允许"）。
...
| **收款组 - 邵梅琳** | 识别开票日期、我方发票抬头、对方发票抬、发票号、项目摘要、金额；支持拖入文件夹 | `邵梅琳发票识别表.xlsx`（不含图片） |
...
- **前端开发调试**：`web/frontend` 下 `npm run dev`（vite 已代理 `/api` 到 8000，可局域网访问）。
- **局域网其他电脑打不开**：Windows 防火墙会拦截首次绑定 8000 端口，弹出对话框点"允许访问"即可。
